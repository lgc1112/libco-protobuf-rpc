...
Coro::~Coro()
{
    LLOG(nullptr, nullptr, LLBC_LogLevel::Trace, "Delete coro, id:%d, scheduleFlag:[yielded:%s, timeouted:%s, cancelled:%s], times[yield:%lu, resume:%lu]",
                  coroId_,
                  yielded_ ? "true" : "false",
                  timeouted_ ? "true" : "false",
                  cancelled_ ? "true" : "false",
                  yieldTimes_,
                  resumeTimes_);
...
int Coro::SetEntry(const CoroEntry &entry)
{
    if (resumeTimes_ != 0)
    {
        LLOG(nullptr, nullptr, LLBC_LogLevel::Error, "Coro[%d] not allow set entry when coro already resumed[times:%lu]",
...
int Coro::SetArgs(void *args)
{
    if (resumeTimes_ != 0)
    {
        LLOG(nullptr, nullptr, LLBC_LogLevel::Error, "CORO[%d] not allow set args when coro already resumed[times:%lu]",
                      coroId_, resumeTimes_);
        LLBC_SetLastError(LLBC_ERROR_NOT_ALLOW);
...
int Coro::Yield(const LLBC_TimeSpan &timeout)
{
    ...
    if (IsYielded())
    {
        LLOG(nullptr, nullptr, LLBC_LogLevel::Error, "Coro[%d] already yielded", coroId_);
    ...
    if (this != coMgr_->GetCurCoro())
    {
        LLOG(nullptr, nullptr, LLBC_LogLevel::Error, 
            "Not allow yield non-current coro, coro:%d, cur-coro:%d",
            coroId_, coMgr_->GetCurCoro()->coroId_);
        LLBC_SetLastError(LLBC_ERROR_NOT_ALLOW);
    ...
    if (IsMainCoro())
    {
        LLOG(nullptr, nullptr, LLBC_LogLevel::Error, "Not allow yield main coro, coro:%d", coroId_);
        LLBC_SetLastError(LLBC_ERROR_NOT_ALLOW);
    ...
    const auto logLv = IsTimeoutedOrCancelled() ? LLBC_LogLevel::Warn : LLBC_LogLevel::Trace;
    LLOG(nullptr, nullptr, 
            logLv,
            "Yield end, coro:%d, timeouted?:%s, cancelled?:%s, cost:%lld.%03lld us",
            coroId_,
            IsTimeouted() ? "true" : "false",
            IsCancelled() ? "true" : "false",
            costTime.ToMicroSeconds(), costTime.ToNanoSeconds() % 1000);
...
int Coro::Resume(const LLBC_Variant &passData)
{
    ...
    if (!IsYielded())
    {
        LLOG(nullptr, nullptr, LLBC_LogLevel::Error, "Coro %d not yield, resume failed", coroId_);
        LLBC_SetLastError(LLBC_ERROR_NOT_ALLOW);
        return LLBC_FAILED;
    }
    ...
    // 自增resume次数 并 Log
    ++resumeTimes_;
    LLOG(nullptr, nullptr, LLBC_LogLevel::Trace, "Resume coro %d, resume times:%lu", coroId_, resumeTimes_);
...
int Coro::Cancel()
{
    if (!IsYielded())
    {
        LLOG(nullptr, nullptr, LLBC_LogLevel::Error, "Coro %d not yield, cancel failed", coroId_);
        LLBC_SetLastError(LLBC_ERROR_NOT_ALLOW);
        return LLBC_FAILED;
    }

    LLOG(nullptr, nullptr, LLBC_LogLevel::Trace, "Cancel coro %d", coroId_);
    timer_.Cancel();
...
{
    LLOG(nullptr, nullptr, LLBC_LogLevel::Warn, "Coro %d timeout", coroId_);
...
void Coro::OnCoroCancel(LLBC_Timer *timer)
{
    ...
    {
        LLOG(nullptr, nullptr, LLBC_LogLevel::Trace, "Coro %d cancel", coroId_);
...
Coro *RpcCoroMgr::CreateCoro(const CoroEntry &entry, void *args, size_t stackSize)
{
    ...
    LLOG(nullptr, nullptr, LLBC_LogLevel::Trace, "Create coro %d", coroId);
...
int RpcCoroMgr::Resume(int coroId, const LLBC_Variant &passData)
{
    ...
    if (!coro)
    {
        LLOG(nullptr, nullptr, LLBC_LogLevel::Error, "Coro %d not found, resume failed", coroId);
...
int RpcCoroMgr::Cancel(int coroId)
{
    ...
    if (!coro)
    {
        LLOG(nullptr, nullptr, LLBC_LogLevel::Error, "Coro %d not found, cancel failed", coroId);
        LLBC_SetLastError(LLBC_ERROR_INVALID);
        return LLBC_FAILED;
    }
...
{
    LLOG(nullptr, nullptr, LLBC_LogLevel::Warn, "Stop");
...
void RpcCoroMgr::Update()
{
    ...
    LLOG(nullptr, nullptr, LLBC_LogLevel::Trace, "Delete all waiting for release coros, size:%lu", waitingForRecycleCoros_.size());
...
{
    if (!waitingForRecycleCoros_.insert(std::make_pair(coro->GetId(), coro)).second)
    {
        LLOG(nullptr, nullptr, LLBC_LogLevel::Error, "Repeated recycle coro, coroId:%d", coro->GetId());
        return;
    }

    coros_.erase(coro->GetId());
    LLOG(nullptr, nullptr, LLBC_LogLevel::Trace, "Recycle coro, coroId:%d", coro->GetId());
}
